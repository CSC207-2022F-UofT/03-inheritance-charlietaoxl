...
class BagMain {
    ...
    public static void enhanceBags(Bag[] bags, boolean double_enhance_handbags) {
        for (Bag curr_bag : bags) {
            curr_bag.enhance();
            if (double_enhance_handbags && curr_bag instanceof HandBag) {
                curr_bag.enhance();
            }
        }
    }
    ...
    public static int countCrossbodyStraps(Bag[] bags) {
        int total_straps = 0;
        for (Bag curr_bag : bags) {
            if (curr_bag instanceof CrossbodyBag) {
                total_straps += ((CrossbodyBag) curr_bag).getNumberOfStraps();
            }
        }
        return total_straps;
    }
